...
static int hcsr04_probe(struct platform_device *pdev)
{
	...
	if (data == NULL) {
		pr_err(" %s: data allocation memory failed\n",__func__);
		ret = -ENOMEM;
		goto exit_data_alloc;
	}

	data->gpio_d1a = of_get_named_gpio(np, "gpio-d1a", 0);
	if (data->gpio_d1a < 0) {
		pr_err(" %s: failed to get gpio-d1a\n",__func__);
		goto exit_data_alloc;
	}
	if(gpio_direction_output(data->gpio_d1a, 0)) {
		pr_err(" %s: failed to set out dir in gpio-d1a\n",__func__);
		goto exit_data_alloc;
	}

	data->gpio_d1b = of_get_named_gpio(np, "gpio-d1b", 0);
	if (data->gpio_d1b < 0) {
		pr_err(" %s: failed to get gpio-d1b\n",__func__);
		goto exit_data_alloc;
	}
	if(gpio_direction_input(data->gpio_d1b)) {
		pr_err(" %s: failed to set in dir in gpio_d1b\n",__func__);
		goto exit_data_alloc;
	}

	if (strcmp(pdev->name, JF1) != 0) {
		int gpio_d1e = of_get_named_gpio(np, "gpio-d1e", 0);
		if (gpio_d1e < 0) {
			pr_err(" %s: failed to get gpio-d1e\n",__func__);
			goto exit_data_alloc;
		}
		if(gpio_direction_output(gpio_d1e, 1)) {
			pr_err(" %s: failed to set out dir in gpio_d1e\n",__func__);
			goto exit_data_alloc;
		}
	}
